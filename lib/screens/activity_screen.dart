...
class _ActivityScreenState extends State<ActivityScreen>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.activity.title, style: const TextStyle(color: AppColors.golden, fontWeight: FontWeight.w800, fontSize: 20)),
        centerTitle: true,
        leading: IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context)),
        actions: [
          // Voice toggle
          IconButton(
            icon: Icon(_voiceEnabled ? Icons.volume_up : Icons.volume_off, color: AppColors.golden),
      ...
      child: Column(children: [
        ...
        Container(
          ...
          child: Column(children: [
            ...
            Text(widget.activity.title, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: AppColors.brownDark)),
            const SizedBox(height: 8),
            Text(widget.activity.description, textAlign: TextAlign.center, style: const TextStyle(fontSize: 14, color: AppColors.brownMedium, height: 1.5)),
            const SizedBox(height: 12),
            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                decoration: BoxDecoration(color: AppColors.golden.withOpacity(0.2), borderRadius: BorderRadius.circular(20)),
                child: Text('~${widget.activity.durationMinutes} min', style: const TextStyle(color: AppColors.golden, fontWeight: FontWeight.w700, fontSize: 13)),
              ),
              if (_isStepBased) ...[
        ...
        if (_isActive) ...[
          Text(_formatTime(_elapsedSeconds), style: const TextStyle(fontSize: 36, fontWeight: FontWeight.w900, color: AppColors.golden)),
          const SizedBox(height: 24),
        ],
        if (!_isActive)
          SizedBox(width: double.infinity, height: 56, child: ElevatedButton(
            onPressed: _startActivity,
            style: ElevatedButton.styleFrom(backgroundColor: AppColors.golden, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
            child: const Text('Start Activity', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Colors.white)),
          ))
        else ...[
      ...
      AnimatedBuilder(
        ...
        builder: (context, _) => Container(
          ...
          decoration: BoxDecoration(
            ...
            gradient: RadialGradient(colors: [AppColors.golden.withOpacity(0.3), AppColors.golden.withOpacity(0.1)]),
            border: Border.all(color: AppColors.golden.withOpacity(0.5), width: 3),
            boxShadow: [BoxShadow(color: AppColors.golden.withOpacity(0.2), blurRadius: 30 * _breathAnimation.value, spreadRadius: 10 * _breathAnimation.value)],
          ),
          child: Center(child: Text(_breathPhase, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: AppColors.brownDark))),
        ),
      ),
      const SizedBox(height: 32),
      Text('Cycle ${_breathCycle + 1} of $_totalBreathCycles', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.brownMedium)),
      const SizedBox(height: 32),
      OutlinedButton(
        onPressed: _completeActivity,
        style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 32, vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), side: const BorderSide(color: AppColors.golden)),
        child: const Text('End Early & Complete', style: TextStyle(color: AppColors.golden, fontWeight: FontWeight.w600)),
      ),
    ]);
  }
  ...
  Widget _journalingView() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('Write about 3 things you\'re grateful for:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.brownDark)),
      const SizedBox(height: 16),
      TextField(
        controller: _journalController, maxLines: 10,
        decoration: InputDecoration(
          hintText: '1. I\'m grateful for...\n\n2. I appreciate...\n\n3. I\'m thankful for...',
          hintStyle: TextStyle(color: AppColors.brownMedium.withOpacity(0.5)),
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: const BorderSide(color: AppColors.golden)),
          enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: AppColors.golden.withOpacity(0.5))),
      ...
      SizedBox(width: double.infinity, height: 52, child: ElevatedButton(
        onPressed: _completeActivity,
        style: ElevatedButton.styleFrom(backgroundColor: AppColors.golden, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
        child: const Text('Save & Complete', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Colors.white)),
      )),
    ]);
  }
  ...
  Widget _stepView() {
    ...
    return Column(children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text('Step ${_currentStep + 1} of ${steps.length}', style: const TextStyle(fontSize: 13, color: AppColors.brownMedium, fontWeight: FontWeight.w600)),
        // Countdown indicator
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
          decoration: BoxDecoration(color: AppColors.golden.withOpacity(0.12), borderRadius: BorderRadius.circular(10)),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            const Icon(Icons.timer, size: 14, color: AppColors.golden),
            const SizedBox(width: 4),
            Text('${_stepCountdown}s', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.golden)),
          ]),
        ),
      ]),
            ...
            if (_voiceEnabled)
              Padding(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Icon(Icons.graphic_eq, size: 18, color: AppColors.golden.withOpacity(0.6)),
                  const SizedBox(width: 6),
                  Text('Listening...', style: TextStyle(fontSize: 12, color: AppColors.golden.withOpacity(0.6), fontWeight: FontWeight.w500)),
                ]),
              ),
            Text(
              steps[_currentStep],
              textAlign: TextAlign.center,
              style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w600, color: AppColors.brownDark, height: 1.6),
            ),
          ]),
        ),
      ),
      ...
      Row(children: [
        ...
        Expanded(
          child: OutlinedButton.icon(
            ...
            style: OutlinedButton.styleFrom(
              ...
              side: BorderSide(color: _currentStep > 0 ? AppColors.golden : AppColors.golden.withOpacity(0.3)),
            ),
          ),
        ),
        ...
        Expanded(
          ...
          child: ElevatedButton.icon(
            ...
            icon: Icon(_currentStep < steps.length - 1 ? Icons.skip_next : Icons.check_circle, size: 18, color: Colors.white),
            label: Text(
      ...
      TextButton(
        onPressed: _completeActivity,
        child: const Text('End Early & Complete', style: TextStyle(color: AppColors.brownMedium, fontWeight: FontWeight.w600, fontSize: 13)),
      ),
    ]);
  }
  ...
  Widget _completedView() {
    return Center(child: Padding(padding: const EdgeInsets.all(32), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(width: 100, height: 100, decoration: BoxDecoration(color: AppColors.golden.withOpacity(0.15), shape: BoxShape.circle), child: const Icon(Icons.check_circle, size: 64, color: AppColors.golden)),
      const SizedBox(height: 24),
      const Text('Well Done! 🎉', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: AppColors.brownDark)),
      const SizedBox(height: 12),
      Text('You completed ${widget.activity.title} in ${_formatTime(_elapsedSeconds)}', textAlign: TextAlign.center, style: const TextStyle(fontSize: 15, color: AppColors.brownMedium, height: 1.5)),
      const SizedBox(height: 8),
      const Text('Taking time for self-care is a sign of strength.', textAlign: TextAlign.center, style: TextStyle(fontSize: 14, color: AppColors.brownLight, fontStyle: FontStyle.italic)),
